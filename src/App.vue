			...
			<div v-if="isNotShareMode" style="margin-left: auto;margin-right: 12px;display: flex;align-items: center;justify-content: end;width: 70%;">
				<div style="width: 50%;" class="mini-screen-mide">
					<el-input :placeholder="$t('FileUrl')" v-model="fileUrl" class="input-with-select">
						<el-button slot="append" :loading="fileUrlLoading" @click="getProjectFromUrl">{{ $t('RequestLoad') }}</el-button>
					</el-input>
				</div>
				<div class="nav-item mini-screen-mide">
					<el-button @click="$refs.readFile.click()" type="text" icon="el-icon-folder-opened" class="nav-item-color">{{ $t('LocalFile') }}</el-button>
					<input type="file" style="display: none" accept=".json,.txt" ref="readFile" @change="getProjectFromFile" />
				</div>
				<div class="nav-item" @click="switchToRequestView"><el-button type="text" icon="el-icon-link" class="nav-item-color">请求测试</el-button></div>
				<div class="nav-item">
			...
			<el-aside width="400px" style="overflow: auto;height: 100vh;" v-show="asideCollapse">
				...
				<div class="mini-screen-show">
					...
					<div style="margin-left: 5px;">
						<el-button @click="$refs.readFile.click()" icon="el-icon-folder-opened">{{ $t('LocalFile') }}</el-button>
						<input type="file" style="display: none" accept=".json,.txt" ref="readFile" @change="getProjectFromFile" />
					</div>
				</div>
				<!-- 接口与分组 -->
				<OamTree
					:data="groups"
					:selectHandler="apiSelectHandler"
					:selectId="selectApiId"
					:emptyText="$t('NoApi')"
					v-loading="groupsIsLoading"
					:element-loading-text="$t('LoadingApiGroups')"
				></OamTree>
			</el-aside>
			<el-main style="height: 100vh;overflow-y: auto;">
				...
				<div>
					...
					<div v-show="mainShowMode == 'PROJECT'">
						<OamProjectView
							ref="projectView"
							@selectServer="projectSelectServer"
							@addCustomServer="projectAddCustomServer"
							@addParamterVariable="projectAddParamterVariable"
							@removeParamterVariable="projectRemoveParamterVariable"
							@saveParamterVariable="projectSaveParamterVariable"
						></OamProjectView>
					</div>
					<!-- 分组视图 -->
					<div v-show="mainShowMode == 'GROUP'"><OamGroupView :group="selectGroup"></OamGroupView></div>
					<!-- 请求测试视图 -->
					<div v-show="mainShowMode == 'REQUEST_TEST'"><OamRequestTestView :isNotShareMode="isNotShareMode" :sessionToken="sessionToken"></OamRequestTestView></div>
					<!-- API视图 -->
					<div v-show="mainShowMode == 'API'"><OamApiView :api="selectApi" :isNotShareMode="isNotShareMode" :sessionToken="sessionToken"></OamApiView></div>
				</div>
			</el-main>
		</el-container>
	</div>
...
import { getProjectAPI, getProjectShareAPI } from '@/api/Project';
...
export default {
	...
	methods: {
		...
		projectAddParamterVariable() {
			this.project.parameterVariables.push({ in: 'query', name: '', value: '' });
			this.updateProjectViewData();
		},
		/**
			...
			for (let i = 0; i < data.length; i++) {
				...
				if (items != null) {
					for (let a = 0; a < items.length; a++) {
						...
						apis.push({ id: apiId, title: api.title });
						this.groupApiDataStore[apiId] = { type: 1, loaded: 0, data: api };
					}
				}
				this.groupApiDataStore[gid] = { type: 0, loaded: 1, data: data[i] };
