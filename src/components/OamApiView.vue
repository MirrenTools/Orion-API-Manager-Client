				...
				<div class="api-header-item" style="display: flex;flex-wrap: wrap;">
					...
					<div class="flexCenter" style="width: 86%;">
						<el-input :placeholder="$t('InputHostAndPath')" size="mini" v-model="api.path">
							<el-select v-model="api.selectServer" slot="prepend" :placeholder="$t('SelectOrAdd')"
								style="width: 170px;" filterable allow-create>
								<el-option v-for="(s, idx) in api.servers" :key="idx" :label="s.url" :value="s.url">
								</el-option>
							</el-select>
			...
			<div class="prem05 background-color-white">
				...
				<div v-if="api.externalDocs" style="margin-top: 0.5rem;">
					...
					<div>
						...
						<br />
						<a :href="api.externalDocs.url" style="color: #409EFF;"
			...
			<div class="api-body-params">
				<div class="api-body-param-header">
					<div style="display: flex; align-items: center;">
						...
						<span v-if="api.parameters == null || api.parameters.length == 0"
							style="margin-left: 10px;font-size: 80%;color: #909399;">{{ $t('NotNeed') }}</span>
					</div>
					<div style="display: flex;align-items: first baseline;">
						<span style="margin-right: 10px;">{{ $t('RequestType') }}</span>
						<el-select v-model="api.requestType" size="mini" style="width: 200px;"
							:placeholder="$t('SelectOrAdd')" filterable allow-create>
							<el-option v-for="(type, idx) in api.consumes" :key="idx" :label="type" :value="type">
							</el-option>
						</el-select>
					</div>
				</div>
				...
				<div class="api-body-param-path plrrem05">
					<el-table v-if="api.parameters && api.parameters.length > 0" ref="requestParamsTable"
						:data="api.parameters" row-key="tableRowkey" default-expand-all
						:tree-props="{ children: 'items', hasChildren: 'hasChildren' }" tooltip-effect="dark"
						style="width: 100%" :empty-text="$t('RequestParamsEmpty')">
						...
						<el-table-column prop="join" :label="$t('ParamsJoin')" width="50" align="center">
							<template slot-scope="scope">
								<el-checkbox v-if="scope.row.join != null" v-model="scope.row.join"
						...
						<el-table-column prop="value" :label="$t('ParamsValue')" min-width="200">
							<template slot-scope="scope">
								<el-input v-if="scope.row.level == 1" type="textarea"
									:autosize="{ minRows: 1, maxRows: 3 }" :placeholder="$t('InputParamsValue')"
									v-model="scope.row.value" @blur="paramsBlurHandler(scope.row)"></el-input>
							</template>
						</el-table-column>
					...
					<div style="padding-top: 0.5rem;">
						<el-input v-model="api.body" v-show="api.body!=null" type="textarea"
							:autosize="{ minRows: 2, maxRows: 10 }" :placeholder="$t('RequestBody')"></el-input>
					</div>
				</div>
				<!-- 响应数据 -->
				<div class="api-body-result">
					<div v-for="(item, idx) in api.responses" :key="idx">
						<div class="api-body-result-header">
							<div style="display: flex;">{{ $t('ResponseStatusCode') }}: {{ item.status }} -
								{{ item.msg }}
							</div>
						</div>
						<div class="plrrem05">
							<div v-if="item.schema" style="border: 1px solid #EBEEF5;">
								<json-viewer :expand-depth="10" :value="item.schema" />
							</div>
							<div v-show="item.data && item.data.length > 0">
								<el-table :data="item.data" row-key="tableRowkey" border default-expand-all
									:tree-props="{ children: 'items', hasChildren: 'hasChildren' }">
									<el-table-column prop="in" :label="$t('ParamsPosition')" width="150" align="right">
									</el-table-column>
									<el-table-column prop="type" :label="$t('ParamsType')" width="120" align="right">
									</el-table-column>
									<el-table-column prop="name" :label="$t('ParamsName')" width="200">
									</el-table-column>
				...
				<div class="api-body-response-header">
					...
					<div style="display: flex;flex-wrap: wrap;align-items: center;">
						...
						<div style="margin-right: 0.7rem;">
							<el-input v-model.number="api.executeTimeout" :placeholder="$t('Timeout')" size="mini"
								style="width: 80px;"></el-input>
						</div>
						<div>
							<el-button type="primary" @click="execute" :loading="api.executing">
				...
				<div class="api-body-response-body" v-show="api.isSxecute">
					<el-tabs style="width: 100%;" v-model="api.responseTags">
						<el-tab-pane :label="$t('BeautifyData')" name="format">
							<json-viewer :expand-depth="10" :value="api.response" />
						</el-tab-pane>
						...
						<el-tab-pane :label="$t('HeaderInfo')" name="header" style="color: #409eff;">
							<el-divider content-position="left" v-if="api.requestHenders">{{ $t('RequestHeader') }}
							</el-divider>
							...
							<el-divider content-position="left" v-if="api.responseHeaders">{{ $t('ResponseHeader') }}
							</el-divider>
				...
				axios(requestData)
					...
					.catch(err => {
						...
						try {
							...
							if (resp != null) {
								this.api.responseHeaders.status = resp.status;
								if (resp.statusText != '') {
									this.api.responseHeaders.statusText = resp.statusText;
								}
								if (resp.headers != null) {
									for (let key in resp.headers) {
										this.api.responseHeaders[key] = resp.headers[key];
									}
								}
							}
							if (resp != null && resp.data != null) {
								this.api.response = resp.data;
							} else {
								let error = {};
