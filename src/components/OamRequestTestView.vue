		...
		<div class="api-body" :class="['api-body-' + api.method]">
			...
			<div class="api-body-params">
				...
				<div style="padding:0 0.5rem;">
					<el-table ref="customRequestParamsTable" :data="api.customRequestParams" row-key="tableRowkey" tooltip-effect="dark" style="width: 100%" :empty-text="$t('RequestParamsEnpty')">
						<el-table-column prop="join" :label="$t('ParamsJoin')" width="50" align="center">
							<template slot-scope="scope">
								<el-checkbox v-if="scope.row.join != null" v-model="scope.row.join" :id="'custom-parameters-join-' + scope.$index" @click.native.prevent="changeCheckBoxSelect('custom-parameters-join-' + scope.$index, scope.row)"></el-checkbox>
							</template>
						</el-table-column>
						...
						<el-table-column prop="value" :label="$t('ParamsValue')" min-width="200">
							<template slot-scope="scope">
								<el-input v-if="scope.row.type=='object'||scope.row.type=='array'" type="textarea" :autosize="{ minRows: 1, maxRows: 3 }" :placeholder="$t('InputParamsValue')" v-model="scope.row.value" @blur="paramsBlurHandler(scope.row)"></el-input>
								<el-input v-else :placeholder="$t('InputParamsValue')" v-model="scope.row.value" @blur="paramsBlurHandler(scope.row)"></el-input>
							</template>
						</el-table-column>
					</el-table>
					<div style="text-align: center;background-color: #FFF;line-height: 50px;padding: 0.5rem;">
